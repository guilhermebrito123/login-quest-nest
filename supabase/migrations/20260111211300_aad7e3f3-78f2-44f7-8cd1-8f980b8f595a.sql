...
-- Corrigir a função bloquear_edicao_diaria_temporaria() 
-- fazendo cast do enum status_diaria para text na comparação
...
BEGIN
  -- Se o status antigo está na lista de bloqueados (cast para text)
  IF OLD.status::text = ANY(status_bloqueados) THEN
